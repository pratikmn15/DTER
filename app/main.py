...
# Standard window initialization with correct flags
...
# AAA render params - using standard Harfang API
...
# load shaders with standard RenderState parameters
...
# load font with standard parameters
...
# load shader imgui - using standard function names
...
while not keyboard.Down(hg.K_Escape):
    ...
    # Standard pipeline view setup
    view_id = 0
    pass_views = hg.SceneForwardPipelinePassViewId()
    view_id, pass_ids = hg.SubmitSceneToPipeline(view_id, scene, hg.IntRect(0, 0, res_x, res_y), 
                                              True, pipeline, res, pipeline_aaa, aaa_config, current_frame)

    view_state = scene.ComputeCurrentCameraViewState(hg.ComputeAspectRatioX(res_x, res_y))

    # Standard scene pass access
    ...
    # Draw UI - using standard Harfang view setup
    ...
    # Standard window update
    hg.UpdateWindow(win)

# Standard Harfang shutdown sequence
hg.RenderShutdown()
